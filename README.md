...
- YOLO 학습 시 결과 저장 경로를 프로젝트 전용 경로로 변경하여 관리함.
...
## **해석 및 원인**
1. **리샘플링 편향 발생**
    - short, spur 데이터만을 강화했으나, 전체 학습 분표 비율이 깨지면서 <br> → 모델이 배경과 결함 경계를 더 혼동하게 됨 (특히 **FN 증가로 확인**가능함)
2. **데이터 다양성 부족**
    - short, spur 이미지만을 중복시켰으므로, fine-tuning이 오히려 **과적합을 유도**했을 확률이 높음.
3. **freeze 설정 영향**
